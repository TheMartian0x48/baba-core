...
/**
 * @file ds.hpp
 * @brief Main header for baba::ds data structures
 */
...
namespace baba::ds
{
    ...
    */
     ...
     */
        ...
        * @note Not implemented yet
        */
        static Array<T> make(baba::memory::Arena* arena, u32 size)
        {
        ...
        static Array<T> make_with_aligment(baba::memory::Arena* arena, u32 size, std::size_t alignment)
        {
            void* ptr = arena->aligned_alloc(size * sizeof(T), alignment);
            if (ptr == nullptr) {
                throw std::runtime_error("failed to allocate memory");
            }
            return Array<T>{
        ...
        static Array<T> make_with_capacity(baba::memory::Arena* arena, u32 size, u32 capacity)
        {
        ...
        static Array<T>
        make_with_capacity_and_aligment(baba::memory::Arena* arena, u32 size, u32 capacity, std::size_t alignment)
        {
            void* ptr = arena->aligned_alloc(capacity * sizeof(T), alignment);
            if (ptr == nullptr) {
                throw std::runtime_error("failed to allocate memory");
            }
            return Array<T>{
        ...
        */
        ...
        Array<T> copy(baba::memory::Arena* arena) const
        {
            void* ptr = arena->alloc(size * sizeof(T));
            if (ptr == nullptr) {
                throw std::runtime_error("failed to allocate memory");
        ...
        static Array<T> merge(baba::memory::Arena* arena, const Array<T>* a, const Array<T>* b)
        {
        ...
        ds::Pair<Array<T>, Array<T>> split(u32 index)
        {
            if (index > size) {
            ...
            auto left_ptr  = static_cast<T*>(arena->alloc(left_size * sizeof(T)));
            auto right_ptr = static_cast<T*>(arena->alloc(right_size * sizeof(T)));
            ...
            for (u32 i = index + 1; i < size; i++) {
                right.data[i - index - 1] = at(i);
            }
            return {left, right};
        }

        static void swap(Array<T>* a, Array<T>* b) noexcept
        {
            auto size     = a->size;
